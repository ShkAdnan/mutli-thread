...
``` bash
#For Node
Run the app in two different tabs on one tab put some small number in the query parameter like "2", "10"
...
On running bigger number node server will be completety be comsumed after that it wont take even 
samll number and process until the first process is complete
